 ...
 * iOSのログから作成しています
 * ただし、実際にDLしたのはPanamaですが、CaliforniaをDLするよう修正しています。
 */
var MapDownloadSenarioDL_Map = (function () {

    function MapDownloadSenarioDL_Map() {
        this.queue = [];
        this.setDonloaded([301, 302]);
    }

    MapDownloadSenarioManager.MapDownloadSenarioDL_Map = MapDownloadSenarioDL_Map;

    MapDownloadSenarioDL_Map.prototype.setDonloaded = function (regionIDList) {
        try {
    ...
    MapDownloadSenarioDL_Map.prototype.start = function () {
        ...
        for (var index = 2; index < regionIDList.length; index++) {
            var regionID = regionIDList[index];
            for (var i = 1; i < 11; i++) {
                queue.push({
    ...
    return MapDownloadSenarioDL_Map;
})();